...
object ThreeLayer {
  ...
  def create(inputNodes: Int, hiddenNodes: Int, outputNodes: Int, tolerence: Tolerence, learningRate: Double,
             inputWeights: Matrix, outputWeights: Matrix, activation: Double => Double,
             delta: (Matrix, Matrix, Matrix) => Matrix): ThreeLayer = {
  ...
  def create(inputNodes: Int, hiddenNodes: Int, outputNodes: Int, tolerence: Tolerence, learningRate: Double,
             activation: Double => Double, delta: (Matrix, Matrix, Matrix) => Matrix): ThreeLayer = {
    val inputWeights: Matrix = Matrix.createRandom(hiddenNodes, inputNodes, 2, 1)
    val outputWeights: Matrix = Matrix.createRandom(outputNodes, hiddenNodes, 2, 1)

    create(inputNodes, hiddenNodes, outputNodes, tolerence, learningRate, inputWeights, outputWeights, activation, delta)
  }
